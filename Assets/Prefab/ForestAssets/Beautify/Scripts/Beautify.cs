/// <summary>
/// Copyright 2016-2017 Ramiro Oliva (Kronnect) - All rights reserved
/// </summary>
using UnityEngine;
using System;
...
namespace BeautifyEffect {
    ...
    public class Beautify : MonoBehaviour {
								...
								[SerializeField]
								BEAUTIFY_OUTLINE_MODE _outlineMode = BEAUTIFY_OUTLINE_MODE.Depth;
        ...
        Material
								...
								const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint54";
								...
								protected virtual void OnRenderImage (RenderTexture source, RenderTexture destination) {
												...
												RenderTextureDescriptor rtDescBase = source.descriptor;
												...
												bool doFinalBlur = _blur && allowExtraEffects;
												if (renderPass == 0 || doFinalBlur) {
																if (doFinalBlur) {
																				...
																				rtBeauty = RenderTexture.GetTemporary (source.descriptor);
																}
												}

												RenderTexture rtDoF = null;
												if (_depthOfField && allowExtraEffects) {
#if UNITY_EDITOR
												...
												if (allowExtraEffects && (_lensDirt || _bloom || _anamorphicFlares || sunFlareEnabled)) {
																...
																if (_bloom || (_lensDirt && !_anamorphicFlares)) {
																				...
																				if (_quality == BEAUTIFY_QUALITY.BestQuality && _bloomAntiflicker) {
																								Graphics.Blit (useBloomSourceTexture ? bloomSourceTexture : source, rt [0], bMat, 9);
																				} else {
																								Graphics.Blit (useBloomSourceTexture ? bloomSourceTexture : source, rt [0], bMat, 2);
																				}
												...
												if (rtBlurTex != null) {
																if (rtBeauty != null) {
																				...
																				BlurThisDownscaling (source, rtBlurTex);
																}
																BlurThis (rtBlurTex);
																if (_quality == BEAUTIFY_QUALITY.BestQuality && _blurIntensity > 0) {
																				BlurThis (rtBlurTex);
																}
																if (rtBeauty != null) {
								...
								void BlurThis (RenderTexture rt) {
												...
												RenderTexture rt2 = RenderTexture.GetTemporary (desc); // (rt.width, rt.height, 0, rtFormat);
								...
								void BlurThisDownscaling (RenderTexture rt, RenderTexture downscaled) {
												RenderTextureDescriptor desc = rt.descriptor;
								...
								void BlurThisDoF (RenderTexture rt, int renderPass) {
												RenderTextureDescriptor desc = rt.descriptor;
												RenderTexture rt2 = RenderTexture.GetTemporary (desc); // rt.width, rt.height, 0, rtFormat);
												RenderTexture rt3 = RenderTexture.GetTemporary (desc); // (rt.width, rt.height, 0, rtFormat);
